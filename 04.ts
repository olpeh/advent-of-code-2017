...
const getPassPhrases = input => input.split('\n').map(r => r.trim());

const getWords = input => input.split(' ');

const countValid = input =>
  getPassPhrases(input).reduce((accumulator, phrase) => {
    const words = getWords(phrase);
    const uniqueWords = new Set(words);
    return uniqueWords.size === words.length ? accumulator + 1 : accumulator;
  }, 0);

console.log(countValid(input)); // 466
